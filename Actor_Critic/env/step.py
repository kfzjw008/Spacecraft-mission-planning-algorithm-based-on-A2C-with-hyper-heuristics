...
def step(action, pop, dim, ub, lb, fun1, vmax, vmin, maxIter, Best_Pos, states,Best_fitnesss):
    ...
    # 假设动作为1，选择执行GWO
        ...
        time, X, Best_Pos, Best_fitness, IterCurve = GWO(pop, dim, ub, lb, fun1, maxIter, states[0])
        reward = -Best_fitness
    # 假设动作为2，选择执行SCA
    elif action == 2:
        time, X, Best_Pos, Best_fitness, IterCurve = SCA(pop, dim, ub, lb, fun1, maxIter, states[0])
        reward = -Best_fitness
    # 假设动作为3，选择执行TSA
    elif action == 3:
        time,X, Best_Pos, Best_fitness, IterCurve = GWO(pop, dim, ub, lb, fun1, maxIter, states[0])
